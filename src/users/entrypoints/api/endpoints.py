...
from users.domain.models import PricingTariff, UserCreateDTO, UserLoginDTO, UserLoginResponse
...
async def get_tariffs():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка получения тарифа: {str(e)}")
...
async def calculate_cost(
    ...
    try:
        ...
        if items_count > max_items:
            raise HTTPException(status_code=400, detail=f"Превышен лимит товаров: {max_items}")
