...
from base.config import (
    get_model_path, get_preprocessing_path, get_confidence_threshold,
    get_max_price_limit, get_min_price_limit
)
...
class PricingService:
    ...
    def _load_model_and_pipeline(self) -> None:
        ...
        if self.model_path.exists() and catboost_available:
            ...
            print(f"❌ Модель не найдена по пути {self.model_path} или CatBoost недоступен")
            self.model = None
    ...
    def _preprocess_single_item(self, product_data: Dict[str, Any]) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            tfidf_name_features = self.preprocessing_pipeline["tfidf_name"].transform(df["name"]).toarray()
            tfidf_desc_features = self.preprocessing_pipeline["tfidf_desc"].transform(df["item_description"]).toarray()
            ...
            df["brand_enc"] = self._safe_transform(self.preprocessing_pipeline["le_brand"], df["brand_name"])
            df["cat_main_enc"] = self._safe_transform(self.preprocessing_pipeline["le_cat_main"], df["cat_main"])
            df["cat_sub_enc"] = self._safe_transform(self.preprocessing_pipeline["le_cat_sub"], df["cat_sub"])
    ...
    def _safe_transform(self, encoder: LabelEncoder, values: pd.Series) -> pd.Series:
        ...
        try:
            ...
            values_safe = values.apply(lambda x: x if x in known_classes else encoder.classes_[0])
            return encoder.transform(values_safe)

    def _calculate_confidence_score(self, prediction: float, features: pd.DataFrame) -> float:
    ...
    def _get_category_analysis(self, product_data: Dict[str, Any], prediction: float) -> Dict[str, str]:
        ...
        if prediction < 10:
            ...
            analysis["recommendation"] = "Рассмотрите возможность повышения качества описания"
        elif prediction < 50:
            ...
            analysis["recommendation"] = "Убедитесь в высоком качестве товара и детальном описании"

        # Специфичные рекомендации по категориям
        if "Electronics" in main_cat:
            analysis["recommendation"] += ". Для электроники важно указать точную модель и состояние"
        elif "Beauty" in main_cat:
            analysis["recommendation"] += ". В косметике важны срок годности и подлинность бренда"
