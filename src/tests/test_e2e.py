...
class TestE2EEndpointsExist:
    ...
    def test_auth_endpoints_without_db_dependency(self, client):
        ...
        for endpoint, method in auth_endpoints:
            ...
            assert response.status_code not in [
                404,
                405,
            ], f"Endpoint {method} {endpoint} имеет проблемы маршрутизации"
            # Обычно 401/403/422 для защищенных endpoints
            if response.status_code in [401, 403, 422]:
                print(f"✅ {method} {endpoint} корректно требует авторизацию")
            else:
                print(
                    f"⚠️ {method} {endpoint} код: {response.status_code} (может быть проблема с БД)"
                )
    ...
    def test_calculate_cost_business_logic(self, client):
        ...
        response = client.post(
            f"/api/v1/users/calculate-cost/?items_count={normal_count}"
        )
        assert response.status_code == 200
        ...
        response = client.post(
            f"/api/v1/users/calculate-cost/?items_count={bulk_count}"
        )
        assert response.status_code == 200
    ...
    def test_excel_template_generation_resilient(self, client):
        ...
        if response.status_code == 200:
            ...
            print(
                f"⚠️ Excel endpoint существует, но есть проблемы: {response.status_code}"
            )
        ...
        with patch("src.base.dependencies.get_token_from_header") as mock_auth:
            ...
            for endpoint, method in protected_endpoints:
                ...
                try:
                    ...
                    print(
                        f"✅ {method} {endpoint} работает с авторизацией (код: {response.status_code})"
                    )
...
class TestE2EPerformance:
    ...
    def test_api_response_times(self, client):
        ...
        for endpoint in fast_endpoints:
            ...
            try:
                ...
                assert (
                    response_time < 2.0
                ), f"Endpoint {endpoint} слишком медленный: {response_time:.2f}s"
    ...
    def test_excel_generation_performance_if_working(self, client):
        ...
        if response.status_code == 200:
            ...
            assert (
                generation_time < 5.0
            ), f"Excel генерируется слишком медленно: {generation_time:.2f}s"
            print(f"✅ Excel генерируется за {generation_time:.3f}s")
        else:
            print(
                f"⚠️ Excel endpoint недоступен (код: {response.status_code}), время: {generation_time:.3f}s"
            )
...
class TestE2ECoverageValidation:
    ...
    def test_business_rules_validation(self, client):
        ...
        assert (
            max_items >= bulk_threshold
        ), "Максимум товаров должен быть >= порога скидки"

        # Проверяем что скидка действительно выгодна
        bulk_test_count = bulk_threshold + 1
        response = client.post(
            f"/api/v1/users/calculate-cost/?items_count={bulk_test_count}"
        )
        ...
        assert (
            actual_with_discount < expected_without_discount
        ), "Скидка должна уменьшать стоимость"
        ...
        assert (
            abs(discount_amount - expected_discount) < 0.01
        ), "Размер скидки неправильный"
    ...
    def test_endpoint_consistency(self, client):
        ...
        test_counts = [
            1,
            5,
            tariffs["bulk_discount_threshold"],
            tariffs["bulk_discount_threshold"] + 5,
        ]
        ...
        for count in test_counts:
            ...
            assert (
                abs(total_cost - expected_total) < 0.01
            ), f"Математика неверна для {count} товаров"
